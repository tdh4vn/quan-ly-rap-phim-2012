...
<%
	...
	System.out.println("aaaaa" + mp3);
	String soghe1 = request.getParameter("soGhe");
	String ngayc1 = request.getParameter("ngayc");
	System.out.println("muon an oi xi muoi" + ngayc1);
	String gioc1 = request.getParameter("gioc");
	String phutc1 = request.getParameter("phutc");
	String tenphim1 = request.getParameter("tenp");
	String lChieu1 = request.getParameter("lChieu");
	System.out.println("ma lich chieu truyen qua" + lChieu1);
	
    String maLichChieu = ""+session.getAttribute("maLichChieu");
    String maXuatChieu = ""+session.getAttribute("maXuatChieu");
%>
...
<table width="100%">
	<tr>
		...
		<fieldset><Legend> <font face="Arial "
			color="black" size="5"> Thông tin ghế </font> </Legend>
		<div align="center">
		<table width=80%>
			<tr class="ourCompany-list">
				<td align=left>Mã phòng :</td>
				<td align=left><label><%=mp3%></label> <input type="hidden"
					name="maPhongName" value="<%=mp3%>" /></td>
			<tr class="ourCompany-list">
				<td align=left>Tên phim :</td>
				<td align=left><label><%=session.getAttribute("tenPhim3")%></label>
				<input type="hidden" name="tenPhimName"
					value="<%=session.getAttribute("tenPhim3")%>" /></td>
			</tr>
			<tr class="ourCompany-list">
				<td align=left>Ngày Chiếu :</td>
				<td align=left><label><%=ngayc1%></label> <input type="hidden"
					name="ngayChieuname" value="<%=ngayc1%>" /></td>
			<tr class="ourCompany-list">
				<td align=left>Xuất Chiếu :</td>
				<td align=left><label><%=gioc1%>h<%=phutc1%></label> <input
					type="hidden" name="gioChieuName" value="<%=gioc1%>" /> <input
					type="hidden" name="phutChieuName" value="<%=phutc1%>" /></td>
			</tr>
			<tr class="ourCompany-list">
				<td width=40% align=left>Số ghế :</td>
				<td align=left><label><%=soghe1%></label><input type="hidden"
					name="soGheName" value="<%=soghe1%>" /></td>
			</tr>
			<tr class="ourCompany-list">
				...
				<%
					String sql5 = "Select TrangThaiGhe from Ghe_LichChieu where SoGhe = '"
							+ soghe1
							+ "' and MaLichChieu ='"
							+ session.getAttribute("lc")
							+ "' and MaXuatChieu = '"
							+ session.getAttribute("xc") + "'";
					ResultSet rs5 = st.executeQuery(sql5);
					while (rs5.next()) {
				%>
				<td align=left><%=rs5.getString(1)%></td>
				<%
					}
				%>
			</tr>
			<tr class="ourCompany-list">
				...
				<td align=left><select name="tensukien" id="tenSuKien"
					onchange="changeTenSuKien();"
 ...
 %> <input type="hidden" id="<%=maSuKienArray.get(i)%>Hidden"
					name="<%=maSuKienArray.get(i)%>Hidden"
 ...
 %>
			...
			<tr class="ourCompany-list">
				...
				<%
					String sql4 = "select Ghe.MaLoaiGhe from Ghe_LichChieu join LichChieu on LichChieu.MaLichChieu=Ghe_LichChieu.MaLichChieu join Ghe on Ghe_LichChieu.SoGhe=Ghe.SoGhe and LichChieu.MaPhong=Ghe.MaPhong where LichChieu.MaPhong ='"
							+ mp3
			...
			<tr class="ourCompany-list">
				...
				<td align=left><label id="tongTienId"></label> <%
 	}
 %>
				</td>
			</tr>
			<tr class="ourCompany-list">
				<td align=center><input type="submit" name="banve" value="OK"
					onClick="return show_confirm()" /></td>

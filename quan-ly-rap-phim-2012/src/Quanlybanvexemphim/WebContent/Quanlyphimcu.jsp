...
<table>
	...
	<tr>
		...
		<table border="1" width="80%" align="center">
			...
			<tr class="ourCompany-list">
				<td><font color="black"> <%=rs.getString(1)%> </font> <input type="hidden" size="2%" name="MaPhim"
					value="<%=rs.getString(1)%>" /></td>
				<td><input type="text" name="TenPhim" size="10%" value="<%=rs.getString(2)%>" /></td>
				<td><input type="text" name="MaDangPhim" size="10%" value="<%=rs.getString(3)%>" /></td>
				<td><input type="text" name="MaLoaiPhim" size="10%" value="<%=rs.getString(4)%>" /></td>
			...
			<%
					...
					rs = st.executeQuery(sql);
					System.out.println("cau select all phim" + sql);
					while (rs.next()) {
			%>
			<tr class="ourCompany-list">
				<td>
	...
	<table width = "100%">
	<tr>
		<td align="right"><input type="button" value="Thêm phim" onclick="javascript:window.location='Themphim.jsp';"></td>
		<td align="left"><input type="submit" name="nutCapNhat" value="Cập nhật"></td>
	</tr>
</table>
</form>
</div>

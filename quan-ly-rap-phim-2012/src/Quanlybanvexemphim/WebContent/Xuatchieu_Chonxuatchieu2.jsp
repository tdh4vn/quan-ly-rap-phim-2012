...
while(rs2.next()){%>
			...
			<tr class="ourCompany-list1">
				...
				<td align=center><a href=""
					onclick="chuyenManHinh('Datve2.jsp?<%=session.getAttribute ("maPhim3")%>&<%=session.getAttribute("ngayChieuAttribute")%>&<%=session.getAttribute("tenphimAttribute")%>&gioChieu=<%=rs2.getString(1)%>&phutChieu=<%=rs2.getString(2)%>')">
